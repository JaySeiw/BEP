...
%% Row and the 3rd column of that row %%
...
if NoGoal==0
    ...
    path_to_goal = zeros(1,2);
    while p>1
        dxG= [NodeMatrix(p,1), NodeMatrix(NodeMatrix(p,3),1)];
        dyG= [NodeMatrix(p,2), NodeMatrix(NodeMatrix(p,3),2)];
        path_to_goal(end+1,:) = [NodeMatrix(NodeMatrix(p,3),1), NodeMatrix(NodeMatrix(p,3),2)];
