...
Nodes=4000; %go crazy with the numbers!
...
while i<Nodes+1
    ...
    [Xnew, Ynew, LengthMatrix, Parent, Cost] = NodeCreator_CopyVFrank(Xmax, Ymax, NodeMatrix, Length, i, Nodes);
    ...
    if intersection==0
        ...
        [NodeMatrix] = NodeRewireVFrank (NodeMatrix, Length,i); %something that has to do with i-rows, makes this code retstart itself sometimes at the same i value
        i=i+1;
    end
...
if ~isempty(GoalLengthMatrix)
    GoalParentNode=GoalLengthMatrix( find( GoalLengthMatrix(3) == min(GoalLengthMatrix(3)) ) , 1); %we can probably refine this part here, but it works
    NodeMatrix(end,[3, 4])=[GoalParentNode, NodeMatrix(GoalParentNode,4)];
